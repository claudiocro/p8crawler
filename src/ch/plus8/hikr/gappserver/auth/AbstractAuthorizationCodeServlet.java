package ch.plus8.hikr.gappserver.auth;
...
public abstract class AbstractAuthorizationCodeServlet extends HttpServlet {
  ...
  protected void service(HttpServletRequest req, HttpServletResponse resp)
    ...
    try {
      ...
      resp.sendRedirect(flow.newAuthorizationUrl().setState(state).setRedirectUri(redirectUri).build());
      credential = null;
    } finally {
      lock.unlock();
    }
  ...
  protected abstract String getState(HttpServletRequest req)
  throws ServletException, IOException;
